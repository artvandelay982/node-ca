...
function main () {
  if(program.check) {
    ...
    if(!fs.existsSync(process.cwd()+'/project.json')) {
      ...
      var projectConfigTest = projectConfigModel.schema.validate(require(process.cwd()+'/project.json'));
    ...
    var projectConfigTest = projectConfigModel.schema.validate(config);
    ...
    switch(program.component) {
      case 'router':
        ...
        prompt.get(collectables, function(err, results){
          ...
          var router = projectConfigModel.router.validate(results);
...
function init () {
  ...
  return prompt.get(collectables, function(err, results) {
    ...
    var projectConfig = projectConfigModel.schema.validate(results);
